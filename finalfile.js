...
async function run() {
  const baseUrl =
    "https://app.apollo.io/#/people?finderViewId=5b8050d050a3893c382e9360&contactLabelIds[]=661fbda37de9a3043c9086a6&prospectedByCurrentTeam[]=yes";
  const csvUrl = "Batch6ITCampaign-mypromovideos.csv";
  const email = "salman@webinnoventix.com";
  const password = "ansarisaleena.salman@123";
  ...
  if (totalItems > 0) {
    ...
    async function processPerson(person, newPage) {
      ...
      try {
        ...
        await newPage.waitForSelector("#general_information_card", {
          timeout: 10000,
        });
        console.log(`Found #general_information_card`);

        const emailElements = await newPage.$$eval(
          "#general_information_card",
          (elements) => elements.map((element) => element.innerText)
        );
        const emailRegex =
          /\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b/g;
        let emails = emailElements.flatMap(
          (element) => element.match(emailRegex) || []
        );

        person.emails = emails.length > 0 ? emails : [""];
      } catch (error) {
        console.error(
          `Error processing ${person.name} at ${cleanedUrl}: ${error}`
        );
        person.emails = [""];
      }
    }
